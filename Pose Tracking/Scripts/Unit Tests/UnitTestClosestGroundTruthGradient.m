...
%           
%
% To try:   Closest point within a ROI around the point (last thing tried)
%           Feature matching with another feature detector and descriptor
%           
%           
...
for TRAINING_INDEX=3:8
    ...
    while ASM.iteration_min ~= ASM.global_min
        ...
        for i=1:length(x)
            ...
            pts = repmat( [x(i) y(i)], [length(keypoints) 1] );
            pts = sqrt( sum( ((pts - keypoints) .^ 2)' ) );
            pts = find( pts <= 10 );
            new_kps = keypoints(pts, :);

            candidates = [  candidates, new_kps(idx,:) ];
        end
